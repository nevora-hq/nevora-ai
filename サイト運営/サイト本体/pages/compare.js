...
export async function getStaticProps() {
  ...
  // カテゴリ・困りごと(worry)は絞り込みに使うため、行データにそのまま引き継ぐ。
  const items = [];
...
export default function ComparePage({ items, categories }) {
  ...
  // /compare?worry=prompt や /compare?category=AIツール比較 のようなURLクエリを
  // 初期絞り込み状態として反映する(困りごとページの比較CTA等からの遷移用)。
  useEffect(() => {
  ...
  return (
    <Layout
      title="商品・サービス比較 | AI活用の総合ガイド｜NEVORA"
      description="AI活用の総合ガイド｜NEVORAで紹介している商品・サービスを一覧で比較できるページです。"
      ...
      {items.length === 0 ? (
        ...
        <>
          <div className="compare-filter-bar">
            ...
            <div className="compare-filter-field">
              <label htmlFor="compare-filter-worry">困りごとで絞り込む</label>
              <select id="compare-filter-worry" value={worry} onChange={handleWorryChange}>
                <option value="">すべての困りごと</option>
                {worryItems.map((w) => (
