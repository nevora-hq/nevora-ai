...
const SITE_URL = "https://nevora-ai.vercel.app";
...
function buildPinDescription(fm) {
  ...
  const keywordLine = picked.length
    ...
    : "";
  const tail = `くわしくは「NEVORA|AI活用の総合ガイド」の記事をご覧ください。#${category.replace(
    /[・\s]/g,
    ""
  )}`;
...
function buildPinInPage(opts) {
  ...
  // 「NEVORA|AI活用の総合ガイド」は1行だと帯幅(マスコット分を除く約620px)に
    ...
    for (const { slug, fm } of targets) {
      ...
      try {
        ...
        const result = await page.evaluate(buildPinInPage, {
          ...
          siteName: "NEVORA|AI活用の総合ガイド",
          domain: "nevora-ai.vercel.app",
