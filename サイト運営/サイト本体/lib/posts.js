...
// 本文冒頭に記事専用の折りたたみ目次を埋め込んでいるslug一覧。
// post.hasEmbeddedTocの算出に使い、ページ側の共通目次との二重表示を防ぐ。
...
function normalizeFrontmatter(data, slug) {
  return {
    ...
    // (MedicalDisclaimer.js参照。カテゴリ別allowlist方式は2026-08-10付けで廃止)。
...
export async function getPostBySlug(slug) {
  ...
  // 記事専用のレイアウト分岐は置かない。装飾はfrontmatterの汎用オプション
  // (sectionAlternate等)とchart/diagram typeで表現する。
