...
**このドキュメントの対象**: `lib/posts.js` の `renderChartHtml` から呼ばれる各chart type(bar/donut/lineChart/prosCons/compareCards等)、および記事専用モジュール(`lib\*Widgets.js`/`*Extras.js`、CONTRIBUTING.mdルール2により新規追加は禁止だが既存分は対象)の図解・グラフ描画関数。
...
フェーズ0監査で、`lib/foundationCompareExtras.js` のように`aria-label`が0件のファイルが確認されているが、これはチャートではなくセクション装飾用のSVGであり実害は小さいと判断されている(`docs/audit-report.md` 観点5)。将来的な機械的な洗い出し(`lib/*.js`内の`aria-label`未設定箇所)はフェーズ7以降のバックログとする。
