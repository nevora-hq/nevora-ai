...
> **⚠️ 2026-08-28 時点で内容が古い(AIサイトへの移行に伴う棚卸し待ち)**
>
> 本ファイルの数値・基準記事3本は、旧・美容サイト時代の記事群を対象に集計したものです。
> AIサイトへの移行で記事はすべて削除され、`lib/posts.js`の記事専用分岐も撤去されたため、
> §0.3(基準記事)・§7(構成実測値)・記法の使用頻度は現状と一致しません。
> AIサイトの記事が数本公開された段階で、`nevora-spec-extractor`を実行して全面的に更新すること。
> §1(独自記法)・§2(コンポーネント)・§9(ビルドコマンド)などサイト実装由来の記述は引き続き有効です。
